 ...
 */
namespace FacturaScripts\Plugins\CloudGallery\Lib\ExtendedController;
 ...
 */
class ListGalleryView extends Lib\ExtendedController\BaseView
{

    use \FacturaScripts\Core\Lib\ExtendedController\ListViewFiltersTrait;

    const DEFAULT_TEMPLATE = 'Master/ListGalleryView.html.twig';
     ...
     * @return ColumnItem[]
     */
    public function getTooltips($model)
    {
        $html = 'rel="tooltip" data-html="true" title="';
        foreach ($this->getColumns() as $column)
            $html .= $column->getTooltips($model);
    ...
    {
        AssetManager::add('css', \FS_ROUTE . '/Plugins/CloudGallery/Assets/CSS/CloudGallery.css');
        AssetManager::add('js', \FS_ROUTE . '/Dinamic/Assets/JS/ListView.js');
        AssetManager::add('js', \FS_ROUTE . '/Plugins/CloudGallery/Assets/JS/CloudGallery.js');
    }
     ...
     * @return string
     */
    public function getPrevisualizedImage($model, $path)
    {
        $html = '';
        switch ($model->GrupoArchivo()->name) {
            case 'Imagen':
                $html = $this->getImageHtml($model);
                break;
            
            default:
                $html = $this->getIconHtml($model);
                break;
        }
     ...
     * @param string $orderKey
     */
    protected function getImageHtml($model)
    {
        if($model->getExtension() !== 'psd')
            return '<img src="'. $this->getUri() . $model->path .'" class="card-img-top img-fluid" style="max-width:250px; max-height:100%; margin: auto;" alt="'. $model->filename .'" title="'. $model->filename .'">';
        else
            return $this->getIconHtml($model);
    }
     ...
     * @param string $orderKey
     */
    protected function getIconHtml($model)
    {
        return '<div class="card-img-top img-fluid text-center fa-6x" alt="'. $model->filename .'" title="'. $model->filename .'"><i class="'. $model->DatosArchivo()->icon .'"></i></div>';
    }
    ...
    {
        $link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on') ? "http://".$_SERVER['HTTP_HOST'] : "http://".$_SERVER['HTTP_HOST']; 
        return $link .str_replace("index.php", "", $_SERVER['PHP_SELF']);
    }
}
