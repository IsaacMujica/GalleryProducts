 ...
 */
class BaseWidget extends CoreWidget\VisualItem
{
     ...
     * @return string
     */
    public function cardGalleryValue($model, $title, $display = 'left')
    {
        $this->setValue($model);
        $class = $this->combineClasses($this->tableCellClass('text-' . $display), $this->class);
        return '<p class="' . $class . '">' . $this->onclickHtml($this->show()) . '</p>';
    }
     ...
     * @return string
     */
    public function tooltipTitleValue($model, $title = '')
    {
        $this->setValue($model);
        return '<p><b>'. $title .':</b> '. $this->value .'</p>';
    }
}
